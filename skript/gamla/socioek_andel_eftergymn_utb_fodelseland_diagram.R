...
utbniva <- px_df %>%
  filter(between(as.numeric(åldernum), 25, 64)) %>%   # ta bara med de som är i åldern 25-64 år
  filter(år %in% unique(px_df$år)[(length(unique(px_df$år))- 4):length(unique(px_df$år))]) %>% 
...
utbniva_tot3 <- utbniva_tot2 %>% 
  filter(utbildningsnivå == "Eftergymn 3+ år") %>% 
  mutate(kön = tolower(kön),
         procent = round(andel * 100, 1)) %>% 
  unite("grupp", c(lansnamn, kön), remove = FALSE, sep = "\n")

utbniva_tot3$grupp <- factor(utbniva_tot3$grupp, levels = c("Dalarna\nkvinnor", "Dalarna\nmän", 
                                                            "Gävleborg\nkvinnor", "Gävleborg\nmän",
                                                            "Värmland\nkvinnor", "Värmland\nmän",
                                                             "Norra Mellansverige\nkvinnor", "Norra Mellansverige\nmän", 
                                                            "Riket\nkvinnor", "Riket\nmän")) 

dia_filnamn <- "diagram_6_utbniva_tidsserie.png"
